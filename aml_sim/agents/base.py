...
class BaseAMLAgent(TraderAgent):
    ...
    # ------------------------------------------------------------------
    # Upstream: slow strategist builder
    # ------------------------------------------------------------------
    ...
    def _apply_risk_overrides(self, proposal: Mapping[str, Any]) -> None:
        ...
        Integer-typed fields (max_order_rate, cooldown_ticks,
        max_consecutive_rejections) are cast to int to prevent silent
        truncation from float values.
        """
        overrides = proposal.get("risk_overrides")
        if not isinstance(overrides, Mapping):
            return
        _INT_FIELDS = {"max_order_rate", "max_consecutive_rejections", "cooldown_ticks"}
            ...
            if key in overrides and hasattr(self.risk_manager, key):
                try:
                    ...
                    if key in _INT_FIELDS:
                        setattr(self.risk_manager, key, max(1, int(value)))
                    else:
    ...
    def _validate_or_keep(self, proposal: Any, *, is_initial: bool = False) -> Any:
        ...
        # Accept both "parameter_updates" (enhanced contract) and
        # "strategy_updates" (upstream/OpenAI contract).
        if isinstance(proposal, Mapping):
            param_updates = proposal.get("parameter_updates") or proposal.get("strategy_updates")
            if param_updates is None:
                param_updates = {}
        else:
            param_updates = None

        if isinstance(proposal, Mapping) and param_updates is not None:
    ...
    async def place_order(
        ...
        # --- Risk gate ---
        last_price = self.prices.get(instrument) if hasattr(self, "prices") else None
